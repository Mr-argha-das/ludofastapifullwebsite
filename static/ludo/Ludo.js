...
export class Ludo {
    ...
    setPiecePosition(player, piece, newPosition) {
        ...
        console.log(this.tempPositions + " ================= " + this.currentPositions);
        console.log(this.tempPositions);
        console.log(this.currentPositions);
        if(this.tempPositions[player][piece] !== this.currentPositions[player][piece]){
    ...
    checkForKill(player, piece) {
        ...
        [0, 1, 2, 3].forEach(opponentPiece => {
            ...
            if (currentPosition === opponentPosition && !SAFE_POSITIONS.includes(currentPosition)) {
                ...
                this.sendKillData(opponent, this.diceValue, this.turn, opponentPiece, BASE_POSITIONS[opponent][opponentPiece]);
    ...
    connectWebSocket() {
        ...
        this.socket = new WebSocket(`ws://127.0.0.1:8080/find-opponent/${userid}/${priceid}`);
            ...
            if (data.message === 'game data' && data.data_received) {
                ...
                if(data.data_received.message === "next turn"){
                   ...
                   this.diceValue = data.data_received.diceValue;
                    ...
                    this.turn = data.data_received.nextTurn;
                    this.setPiecePosition(data.data_received.opponent, Number(data.data_received.opponentPiece), BASE_POSITIONS[data.data_received.opponent][Number(data.data_received.opponentPiece)]);
                } else{
    ...
    handleGameData(dataReceived) {
        ...
        console.log(data)
        this.setPiecePosition(`P${dataReceived.nextTurn+1}`, Number(dataReceived.piece), data[`P${dataReceived.nextTurn+1}`][Number(dataReceived.piece)]);
